...
- **Flag discrepancies, don't silently fix them** - if the repo's own docs/naming disagree with what the code actually does, call it out (e.g. "the repo's CLAUDE.md still references an older layout; the live code is X"). The section name shall be called `Technical Debt` and the table shall be called `Discrepancies`.
